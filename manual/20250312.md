...
src -> components 내부에 Header 폴더 생성 -> Header2.jsx / Header.css 생성

그리고 현재 App.jsx에 Header 컴포넌트 관련 코드들이 전부 다 합쳐져 있는 상태. index.css 내부에 Header 컴포넌트 관련 css 코드도 작성

이를 완전히 분할하여 Header 관련 컴포넌트를 Header2.jsx / Header.css 로 분리

지금 현재와 동일하게 페이지가 구현될 수 있도록 연구

### task # 5 풀이

* 순서가 큰 의미가 있는 것이 아님

- index.css 파일에서 header 관련 파트를 전부 다 잘라내서 Header.css 파일로 이동 -> index.css에는 header 태그 관련 css 코드가 없으므로 default 설정으로 넘어가게 됩니다.(일부 상속 받은 스타일이 적용될 수 있음)

- App.jsx에서 Header 컴포넌트 관련 코드를 잘라내서 Header2.jsx 파일로 이동 -> Header 컴포넌트를 다 잘라내서 붙여넣기를 하고 나서 보니까 코드가 비어있는 부분이 많음

getRandomInt() 함수를 호출하고 있는데, 걔가 없음 -> 잘라와야함

그리고 또 확인하니 reactDescriptionsEng, reactDescriptionsKor 이 없다는 것이 확인됨 -> 잘라와야 함

그리고 또 리턴 파트 확인 해보니 reactImg 변수를 가지고 온 것이 확인되는데, 걔가 없습니다.(import 문)

-> 이미지 경로를 그대로 가지고 올 경우에 오류가 발생 src/components/Header/Header2.jsx였기 때문

```jsx
import reactImg from '/src/assets/react-core-concepts.png';
// import reactImg from '../../assets/react-core-concepts.png';
// 상위 폴더의 상위폴더로 들어간 다음에 해당 파일을 찾아야 하기 때문에
import "./Header.css";  // 같은 폴더 내에 있기 때문에 
```

components 내에 각 Component 별 jsx / css 을 묶어서 작성하시는 방법이 있고, Components 내에 css 폴더를 만든 다음 css만 모아두는 경우도 있습니다. 이는 작성 방식에 따라 다른데, 저희는 springboot 에서는 css 폴더를 다 모아두면서 작성을 했었기 때문에 이번에 컴포넌트별 분할 방식으로 작성

여기서 유의할 점은 .jsx 파일명의 경우 관례적으로 Component 명과 동일하게 작성하는 편

## task # 5를 통해서 저희가 발전한 부분

: App.jsx에 통짜로 짜던 부분을 어떻게 분할할 수 있을까에 해당하는 부분

즉, Component 개념을 학습을 하면서 재사용이 가능하다 라고 이야기했지만 -> 실질적으로 그러한 방법으로 기획하지 않는 이상 솔직히 말해서 한번 쓰고 버려지는 Component도 많습니다.

근데 이 부분을 강조하면 Component의 장점 자체가 사라지기 때문에 강사님이 굳이 여러번 사용하는 예시를 두개 쯤 넣음(영어로 페이지로 보여드림)
...
즉 CoreConepts 컴포넌트 자체는 4번 반복하는 형태로 작성할 예정이지만 return 내부가 서로 다르게 정의되어야지만 재사용이 가능하다고 볼 수 있음

마치 다른 데이터로 JS 기능을 구축하고 재사용할 수 있듯이 리액트에서도 컴포넌트에 서로 다른 데이터를 집어넣어 재사용하고 렌더링 할 수 있습니다. -> 이때 이용되는 개념이 props에 해당
...
만약에 CoreConepts 라는 컴포넌트를 이상과 같이 정의만 하고, 호출하지 않았다면 사실상 매개변수인 props는 지정되지 않았다고 볼 수 있음. 그리고 그 props는 return에 처음 정해지 img/title/description 이라는 속성명을 지닌다고 할 수 있음
...
과  같이 작성할 수 있는데 -> 첫번째 컴포넌트는 오류가 발생
-> desc 라는 속성명은 없기 때문에
